...
**Suno has a 1000-character limit.** Each prompt is the **Shared Block** (identity) + the **track description**. The shared block is 474 chars, leaving ~524 chars per track for mood, mode, BPM, acoustic, and motif placement. All 35 prompts below verified to combine to under 970 chars.

## Existing legacy tracks (pre-prompts)
...
**Why this is shorter than the previous shared block:** the old version locked every track to D Dorian / Mixolydian and a "stone guild hall" room sound. That worked for the bright tracks (town, guildhall, dawn) but fought against the dramatic ones (Rat Plague, Inquisition, Game Over). The leitmotif (D-C-A-G) is the real unifier — it works in any key. Per-track mode + acoustic specs let each piece reach for the emotion it actually needs.

---

# Sleeping Quarters (room) — example of full Suno paste
...
### Chapter 4 — The Name
Proud, ceremonial. Slow lute and harp unison over a held cittern drone. A procession through a grateful town. **Mode:** D Mixolydian (bright but rooted) — **BPM:** 65 — **Acoustic:** outdoor stone square, grand open reverb. The guild motif played clearly by the full ensemble — no longer hidden, no longer borrowed. Dignified, earned, unhurried. Think: a small ceremony that matters to everyone present.
...
Somber, unresolved. Solo fiddle playing a broken version of the guild motif over a distant cittern drone. The lean-to empty, the guild dissolved. But not quite despair — the motif hints at "another town, another chance." **Mode:** D Aeolian (true minor) — **BPM:** 50 — **Acoustic:** empty cold space, minimal reverb. The guild motif appears broken — only the first three notes (D-C-A), the fourth never arrives. Think: the final page of a story that isn't the final page.

---

## Usage

1. Copy the **Shared Block** (474 chars).
2. Paste it into Suno's prompt field.
3. Add a newline, then paste the track-specific description.
4. Combined total stays under 970 chars for all 35 tracks (verified — longest is Merchant Visit at 492 chars description).
