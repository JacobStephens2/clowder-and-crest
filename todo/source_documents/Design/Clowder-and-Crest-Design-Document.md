# Clowder & Crest — Design Document v9

A cat guild management game set in a medieval fantasy world. Built with Phaser 3 + TypeScript + Capacitor for Android and web.

**What changed from v8:** Explorable town map (8x10 grid with 8 buildings, walkable streets, NPC cats, stray cat recruitment). Brawl minigame with Rat King boss, powerups, virtual joystick with multi-touch. Nonogram puzzle minigame. 7 minigame types total, 2 per job category. Job-location flow (accept at board, walk to building to start). Furniture stat boosts. Starvation game over. Fight music tracks. Procedural job flavor text. PixelLab dog/fish/barrel sprites. Scene memory leak fixes. Multiple save slots. SCENES constant map.
