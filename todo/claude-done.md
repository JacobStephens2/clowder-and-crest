[x] - Build SIGNED release APK at v2.5.27 (versionCode 54) bundling 2 fixes — saved to `todo/clowder-and-crest-2.5.27-release.apk` and uploaded to GitHub release v2.5.27. (1) Day of Rest title-mode synchronous reopen — the previous re-stub fix (bca6bd6/v2.5.25) addressed gameState being cleared but the panel reopen still relied on a 400ms setTimeout fired AFTER `switchScene('TitleScene')`. This raced with TitleScene's create() — and for Pounce specifically (Matter physics teardown takes longer than Arcade), the timing window was unreliable enough that the player could land on a blank scene. New approach: handoff via a module-level flag in Showcase.ts (`setPendingTitleDayOfRestReopen`/`consumePendingTitleDayOfRestReopen`). puzzle-complete and puzzle-quit (title-demo branch) sets the flag BEFORE switchScene; TitleScene.create() ends with `if (consumePendingTitleDayOfRestReopen())` and opens the catalogue inside a 50ms delayedCall (one paint frame). No more setTimeout race. Per user feedback "after i finish pounce in from the title day of rest menu, I click continue and am brought back to a blank town scene rather than the day of rest menu." (2) docs: README + CLAUDE + GitHub repo description bumped from 14 to 15 minigames. README minigame table now lists Roof Scout as #15, intro line + Day of Rest description + music count ("14 music tracks" → "70+ music tracks across every minigame, chapter, room, and event") all updated. CLAUDE.md header + game flow + design-decisions bullet + Capacitor haptics line all bumped. GitHub repo description updated via `gh repo edit` to mention 15 minigame types and added "vertical climbing" to highlights. Per user feedback "update the readme and github description. I think they only mention 14 minigames."
[x] - Build SIGNED release APK at v2.5.26 (versionCode 53) — Pounce real win-detection fix + shorter reload — saved to `todo/clowder-and-crest-2.5.26-debug.apk` and uploaded to GitHub release v2.5.26. Per user feedback "all the rats are on the ground, yet it doesnt register as complete" (second pass on Pounce). The v2.5.24 periodic-check fix was correct in structure but relied on a threshold that never actually fired: Phaser/Matter Y increases downward, the static ground top is at GROUND_Y (650), a rat is a circle with radius 8, so a resting rat's CENTER is at GROUND_Y - 8 = 642 — ABOVE the ground top. The previous threshold `> GROUND_Y - 5` (= 645) required the center to be BELOW 645, but a resting rat at 642 is ABOVE 645, so the check returned false for every successfully knocked rat. New threshold `> GROUND_Y - 30` (= 620) reliably distinguishes "on structure" (all in-place starting positions are at y < 600) from "knocked to ground" (resting rats at 642). Also reduced post-launch settle wait from 2500ms → 1100ms per "make reload time shorter in pounce" — the periodic 500ms checkFallenRats already catches slow domino chains so the settle wait can be much shorter without missing late wins.
[x] - Build new debug APK at v2.5.25 (versionCode 52) — Day of Rest title-mode blank-screen fix — saved to `todo/clowder-and-crest-2.5.25-debug.apk`. Per user feedback "I completed the easy roof scout track from the title day of rest menu, but when i clicked back to day of rest, i came to a blank game screen rather than the title day of rest menu." The previous fix relied on gameState surviving the practice run intact, but enough side-effects (switchToTrackset, endPracticeRun, scene shutdowns, overlay removal) were apparently clearing or invalidating the in-memory state to leave the post-run reopen with no gameState. showDayOfRestPanel returns early when gameState is null, so the player landed on TitleScene with no panel — a "blank game screen" from their perspective. Fix: in the title-demo branch of both puzzle-complete and puzzle-quit, re-create the stub save with createDefaultSave('Demo') + titleDemoState flag and assign it back to gameState BEFORE the switchScene + reopen sequence. Defensive guarantee — the panel is now always called with a fresh non-null gameState regardless of what the practice run did.
[x] - Build new debug APK at v2.5.24 (versionCode 51) bundling 7 fixes — saved to `todo/clowder-and-crest-2.5.24-debug.apk`. (1) Roof Scout tunnel-proof base platform: extended the base from a 12px ledge to a tall collider spanning from spawn to WORLD_HEIGHT, so a fast fall cannot tunnel through anymore. Per "I fell a long distance and fell under the base platform and got stuck there." (2) Patrol prowler hit feedback: layered burst on every successful tap — louder SFX, layered crate_push thud, haptic.medium, 80ms camera shake, expanding/fading purple ring, "Got it!" floating callout. Per "in patrol, give more feedback when i tap and eliminate a prowler." (3) Day of Rest practice result overlay: new `showPracticeResultOverlay` in jobFlow.ts (lightweight star-rating screen with "Back to Day of Rest" button), wired into both puzzle-complete and puzzle-quit handlers. Title-demo runs return to TitleScene + reopen the catalogue with unlockAll=true; in-game runs return to GuildhallScene + reopen the campaign-progress catalogue. Music routing fixed: title-demo runs swap directly to 'title' trackset (not __normal_pool__) so the practice and post-run tracks no longer briefly stack. Per "after i got to the watch point in roof scout in title screen day of rest mode, I got a success message, but then i got stuck", "When a user enters day of rest from the main title screen, when they exit a mini game, take them back to the fully unlocked no save needed day of rest game choice menu", and "after i finished prowler from the day of rest title menu, I think there were two songs playing simultaneously." (4) Pounce periodic win check: the periodic checkFallenRats (every 500ms) now also fires winGame() when the count reaches totalRats, closing the gap where slow domino chains finished after the post-launch 2500ms settling window. Per "I knocked all the rats down to the floor, but the game didn't seem to notice." (5) Group dialogue speaker emphasis: showLine() now applies a CSS transform to the speaking portrait — scale(1.35) + opacity 1 + z-index 5 — and dims the others to scale(0.9) + opacity 0.55. 240ms ease-out transition. Per "in guild moments, increase the size of the speaker's portrait when they are speaking."
[x] - Build new debug APK at v2.5.22 (versionCode 49) bundling 3 fixes — saved to `todo/clowder-and-crest-2.5.22-debug.apk`. (1) Title-screen Day of Rest no longer creates a fully unlocked save: replaced the demo-save load with a minimal `createDefaultSave('Demo')` stub (Day 1, Chapter 1, just the player wildcat), tagged with `flags.titleDemoState = true`. New lightweight `set-transient-game-state` event sets gameState in main.ts WITHOUT firing the full game-loaded side-effect cascade (BGM swap, day timer, offline earnings). The panel opens directly over TitleScene (no navigate). puzzle-complete and puzzle-quit handlers in main.ts check `gameState.flags.titleDemoState` and route back to TitleScene + clear gameState instead of dropping the player into the stub guildhall. The panel close (X) button does the same. Per user feedback "for accessing the day of rest from the title screen, give player access to the minigame menu without creating a fully unlocked save." (2) Roof Scout proximity wall jump now extends each platform's vertical reach by 20px above and below its actual band, so a fast straight-down fall past a thin 12px ledge gives the player a forgiving 50+ ms window to wall-jump off the edge. Per user feedback "if I am falling straight down and cross a surface, such as the edge of a platform, let me wall jump off of it." (3) Wall-jump grace windows widened: WALL_COYOTE_MS 180 → 320 (≈19 frames at 60fps) and JUMP_BUFFER_MS 150 → 220. Per user feedback "make the window of opportunity to wall jump longer."
[x] - Build new debug APK at v2.5.21 (versionCode 48) — new-game prompt portrait + tomcat/queen labels — saved to `todo/clowder-and-crest-2.5.21-debug.apk`. Per codex-todo "show the portrait of the he wildcat if the player chooses that at new game, even before clicking begin, and do the same for the she wildcat if they choose she. and reword the choice to tomcat and queen." The new-game name prompt now shows the matching wildcat portrait (140px, pixelated) above the name input. Defaults to the female portrait (queen is the highlighted starter option) and swaps live when the player taps the other gender button — so they can see the cat they're committing to before clicking Begin. The two gender buttons are relabeled "queen" and "tomcat" with "she / her" / "he / him" pronouns kept as smaller subtitles beneath each label so players who don't know cat-fancier terms still understand which is which. Underlying chosenGender state ('male' | 'female') is unchanged so save data and pronouns helper continue to work without migration.
